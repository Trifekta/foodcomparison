...
export function FoodAppLinks() {
  return (
    <section className="rounded-2xl border border-ink-200 bg-white p-4">
      <h2 className="text-[0.95rem] font-bold text-ink-900">No screenshot yet?</h2>
      ...
      <ol className="mt-3 space-y-2">
        {STEPS.map((step, index) => (
          <li key={step} className="flex gap-2.5">
            <span
              aria-hidden="true"
              className="mt-px flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-brand-100 text-[0.7rem] font-bold tabular-nums text-ink-800"
            >
              {index + 1}
            </span>
            <span className="text-[0.88rem] leading-snug text-slate-600">{step}</span>
          </li>
        ))}
      </ol>
      ...
      <div className="mt-4 grid grid-cols-2 gap-2">
        {FOOD_APPS.map((app) => (
          <a
            ...
            className="flex min-h-12 items-center gap-2.5 rounded-xl border border-ink-200 bg-white px-3 py-2 text-left text-[0.88rem] font-semibold text-ink-900 hover:bg-ink-50"
          >
            <span
              aria-hidden="true"
              className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-lg text-base font-black ${app.tile} ${app.ink}`}
            >
              {app.mark}
            </span>
            <span className="leading-tight">Open {app.name}</span>
          </a>
        ))}
      </div>

      <p className="mt-3 text-[0.78rem] leading-snug text-slate-500">
        Your SnipSavor page will stay ready when you come back.
      </p>
    </section>
  );
}
