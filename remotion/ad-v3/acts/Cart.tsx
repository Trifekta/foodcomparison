...
export function Cart() {
  ...
  const ty = track(frame, [
    ...
    { f: sec(4.3), v: 470 + 150 / 2 - (CARD.y + CARD.h / 2), ease: CURVE.out },
  ]);

  const enter = ramp(frame, 0, sec(0.7), CURVE.out);
  // Gone by the time its contents have left it.
  const fade = 1 - ramp(frame, sec(4.36), sec(4.72));
